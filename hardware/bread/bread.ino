...
const uint8_t crumb_macs[][6] = {
 ...
 // State machine
 // ---------------------------------------------------------------------------
 enum State { INIT, WAIT_FOR_START, DROP_NEXT, TRACK_SIGNAL, DONE, ERROR };
 State st = INIT;
...
void setup() {
  ...
  delay(200);
  Serial.println("Bread ready. Starting drop sequence (crumb A first).");
